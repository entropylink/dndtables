...
- Tabs: Vendors and Job Board are full tabs; Quick NPC, Tavern, Travel Encounters, Weather and Loot are declarative (`DT.simpleTableTab` with context, `only`/`weight`/`when`/`count`, inline dice). The test suite checks every context combination of every declarative tab.
